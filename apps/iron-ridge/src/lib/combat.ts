...
// Implements Iron Ridge Ranged Combat Resolution (Draft v1)
...
function rangeMod(profile: WeaponProfile, effectiveRange: number): number {
  const bands = RANGE_TN[profile];
  for (const band of bands) {
    if (effectiveRange >= band.min && effectiveRange <= band.max) return band.mod;
  }
...
export function inTankDeadZone(
  ac: UnitInstance,
  dc: UnitInstance,
  hm: HeightMap
): boolean {
...
function rollDirectDamage(def: UnitDef): number {
  switch (def.id) {
    case "ww2_rifle_infantry": return rollPool(1, 6);
    case "ww2_machine_gun":    return rollPool(1, 6, 1);
    case "ww2_at_gun":         return rollPool(2, 6, 2);
    case "ww2_tank":           return rollPool(3, 6, 3);
    case "ww2_mortar":         return rollPool(2, 6, 1);
    case "ww2_artillery":      return rollPool(3, 8, 4);
    case "ww2_at_infantry":    return rollPool(2, 6, 2);
    // medieval
    case "med_infantry":       return rollPool(1, 6);
    case "med_archers":        return rollPool(1, 6);
    case "med_horseman":       return rollPool(1, 8);
    case "med_spearman":       return rollPool(1, 6, 1);
    case "med_artillery":      return rollPool(2, 6, 1);
    default:                   return rollPool(1, 6);
  }
}

function rollSplashDamage(def: UnitDef): number {
  switch (def.id) {
    case "ww2_at_gun":      return rollPool(1, 6);
    case "ww2_tank":        return rollPool(2, 6);
    case "ww2_mortar":      return rollPool(2, 6, 1);
    case "ww2_artillery":   return rollPool(3, 8);
    case "ww2_at_infantry": return rollPool(1, 6);
    case "med_artillery":   return rollPool(1, 6);
    default:                return rollPool(1, 6);
  }
}
...
function isHE(def: UnitDef): boolean {
  return (
    ...
    ["ww2_tank", "ww2_at_gun", "ww2_mortar", "ww2_artillery",
     "ww2_at_infantry", "med_artillery"].includes(def.id)
  );
}
...
export function resolveAttack(
  ...
  opts?: {
    ...
    facingMod?: number; // front=+2 side=-1 rear=+1 (to-hit)
    facingDmg?: number; // front=-2 rear=+2
    blindFire?: boolean;
  }
): CombatResult {
