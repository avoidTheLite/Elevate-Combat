...
import { hexDistance, hexToWorld, parseKey } from '../hex.ts';
...
function simArmy(spec: BattleSpec, side: SimSide, at: HexKey): Army {
  ...
  // Only the fields createBattle reads; cast keeps this robust to Army growing.
  return { id: `${team}army-sim`, team, at, units, movesLeft: 0 } as Army;
}
...
export const ironRidgeRuleset: BattleRuleset<BattleSpec, GameState, BattleResult> = {
  ...
  createBattle(spec, grid, seed) {
    ...
    return inSimScope(state, () => {
      ...
      const site = battleSite(world);
      const att = simArmy(spec, 'attacker', site.origin.key);
      const def = simArmy(spec, 'defender', site.target.key);
